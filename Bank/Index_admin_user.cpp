...
//初始化用户列表
void Index_admin_user::init_user_list()
{
	users.clear();
	try
	{
...
//列表选中
void Index_admin_user::lv_members_click(QModelIndex mi)
{
	int index = mi.row();
...
//添加客户
void Index_admin_user::btn_add_user_click()
{
	Edit_user_form* add = new Edit_user_form(this, NULL, 2, verb::post);
	connect(add, &Edit_user_form::edit_complete, this, &Index_admin_user::init_user_list);
	add->show();
}

//删除客户
void Index_admin_user::btn_remove_user_click()
{
...
//编辑客户
void Index_admin_user::btn_edit_user_click()
{
	if (current_user_index >= 0 && current_user_index < users.size())
	{
		Edit_user_form* edit = new Edit_user_form(this, &users[current_user_index], 1, verb::put);
		connect(edit, &Edit_user_form::edit_complete, this, &Index_admin_user::init_user_list);
		edit->show();
	}
}

//显示账户列表
void Index_admin_user::btn_account_list_click()
{
	if (current_user_index >= 0 && current_user_index < users.size())
	{
...
//冻结客户（未完成）
void Index_admin_user::btn_freeze_user_click()
{

}

//解冻客户（未完成）
void Index_admin_user::btn_unfreeze_user_click()
{
